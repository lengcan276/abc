...
class StructureUtils:
            ...
            if len(path_parts) >= 4:
                ...
                if state and conf_name:
                    ...
                    potential_paths = [
                        ...
                        os.path.join(path_parts[:mol_nameidx+1], 'results', f"{state}{conf_name}.xyz"),
    ...
    def calculate_structure_features(mol):
        ...
        try:
            ...
            for group_name, smarts in tadfgroups.items():
                try:
                    pattern = Chem.MolFromSmarts(smarts)
                    if pattern:
                        matches = mol.GetSubstructMatches(pattern)
                        features[f'has{groupname}'] = 1 if matches else 0
                        features[f'count{groupname}'] = len(matches)
                except:
                    features[f'has{groupname}'] = 0
                    features[f'count{group_name}'] = 0
            ...
            for ring_name, smarts in ringsystems.items():
                try:
                    pattern = Chem.MolFromSmarts(smarts)
                    if pattern:
                        matches = mol.GetSubstructMatches(pattern)
                        features[f'has{ringname}'] = 1 if matches else 0
                        features[f'count{ringname}'] = len(matches)
                except:
                    features[f'has{ringname}'] = 0
                    features[f'count{ring_name}'] = 0
